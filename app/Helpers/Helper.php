...
function registrarLogFront($usuarioId, $accion, $descripcion = null)
{
    // Datos del log
    $data = [
        'usuario' => $usuarioId,
    ...
    /*UsuarioLog::create($data);*/

    // 2. Registrar en archivo/CloudWatch (Log de Laravel)
    $logMessage = "Usuario $usuarioId | Acción: $accion | " . ($descripcion ?? 'Sin descripción');

    // Puedes diferenciar niveles según acción
    if (in_array($accion, ['login_fallido', 'bloqueo'])) {
        Log::channel('front')->warning($logMessage, $data);
    } elseif ($accion == 'errors') {
        Log::channel('front')->error($logMessage, $data);
    }else{
