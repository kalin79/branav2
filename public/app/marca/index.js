jQuery(function() {
    ...
    ModalCRUD.create({
        ...
        initialized: function() {
            ...
            $('#avatar_detalle').on('change', function() {
                const file = this.files[0];
                $("#file-avatar-detalle").text(file['name']);
                if (file) {
                    let reader = new FileReader();
                    reader.onload = function(event) {
                        $('#img-upload-avatar').attr('src', event.target.result);
                    }
                    reader.readAsDataURL(file);
                }
            });

            $('#image_detalle').on('change', function() {
                const file = this.files[0];
                $("#file-image-detalle").text(file['name']);
                if (file) {
                    let reader = new FileReader();
                    reader.onload = function(event) {
                        $('#image-detalle-upload').attr('src', event.target.result);
                    }
                    reader.readAsDataURL(file);
                }
            });
