    ...
    @forelse ($categorias as $categoria)
        <tr id="{{$categoria->id}}">
            @if($parent_id==0)
                <td>
                    @if($categoria->poster )
                    <img src="/images/categoria/{{$categoria->id}}/{{ $categoria->poster }}" alt="item.name"
                        width="80px">
                    @endif
                </td>
            @endif
            <td>
                @if ($parent_id==0)
                    <a href="{{route('subcategories.index',$categoria->id)}}">{{ $categoria->titulo }}</a>
                @else
                    {{ $categoria->titulo }}
                @endif
            ...
            @if($parent_id==0)
                <td>{{ $categoria->sub_categories_count }}</td>
            @endif
            <td class="position">{{ $categoria->position }}</td>
...
<tfoot>
    <tr>
        <td colspan="{{$parent_id == 0 ? '4' : '3'}}">{{ $categorias->links() }}</td>
