        ...
        @forelse ($cuidados as $valor)
            <tr id="{{$valor->id}}">
                ...
                <td width="150px">
                    ...
                    <a title="Editar" data-name="{{ $valor->titulo }}"
                        href="{{ route('cuidado-personal.edit', $valor->id) }}"
                    ...
                    @if ($valor->active == 1)
                        <button onclick="desactivar({{ $valor->id }},'{{ route('cuidado-personal.desactive') }}')"
                            title="Desactivar" data-id="{{ $valor->id }}" class="btn btn-outline-warning btn-sm">
                            <i class="fa fa-ban"></i>
                        </button>
                    @else
