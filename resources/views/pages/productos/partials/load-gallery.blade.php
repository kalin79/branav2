    ...
    <tbody>
        @forelse ($gallery as $image)
            <tr id="{{$image->id}}">
                <td>
                    @if($image->image)
                        <img src="{{ asset('storage/images/products') }}/{{ $product->id }}/{{ $image->image }}" width="200px">
                    @endif
                </td>
                <td>
                    {{ $image->order }}
                ...
                <td width="150px">
                    <button type="button"
                        onclick="eliminarImageGallery({{ $image->id }},'{{ route('products.gallery.destroy', [$product->id, $image->id]) }}')"
